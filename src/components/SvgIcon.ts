...
export class SvgIcon extends LitElement {
  ...
  render() {
    ...
    // circle-dot needs special rendering (stroke ring + filled dot)
    if (this.name === 'circle-dot') {
      return html`
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="${icon.viewBox}"
          width="${this.size}" height="${this.size}" aria-hidden="true" focusable="false">
          <path d="${icon.path}" fill="none" stroke="currentColor" stroke-width="32"/>
          <circle cx="256" cy="256" r="96" fill="currentColor"/>
        </svg>
      `;
    }

    // circle-empty needs stroke-only rendering
    if (this.name === 'circle-empty') {
      return html`
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="${icon.viewBox}"
          width="${this.size}" height="${this.size}" aria-hidden="true" focusable="false">
          <path d="${icon.path}" fill="none" stroke="currentColor" stroke-width="32"/>
